...
ggsave("responseswitch.png")
```

The analysis reveal a main effect of Model (Group):

```{r}
mod_feedback <- glmer(ResponseSwitch ~ best.model * BlockTypeCoded + (1|HCPID),  family="binomial", weights=abs(diff.LL), data=full)
...
ggsave("responsetimes.png")
```


```{r}
mod3 <- glmer(RT ~ Model * Block+ (1|HCPID), family="Gamma", weights = abs(deltaLL), adata_block)
tab_model(mod3)
```